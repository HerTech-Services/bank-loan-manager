...
public class DowngradingResource {
    ...
    private final DowngradingQueryService downgradingQueryService;

    public DowngradingResource(DowngradingService downgradingService, DowngradingQueryService downgradingQueryService) {
        this.downgradingService = downgradingService;
        this.downgradingQueryService = downgradingQueryService;
    }
    ...
    public ResponseEntity<List<DowngradingDTO>> getAllDowngradings(DowngradingCriteria criteria, Pageable pageable) {
        log.debug("REST request to get Downgradings by criteria: {}", criteria);
        Page<DowngradingDTO> page = downgradingQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /downgradings/count} : count all the downgradings.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/downgradings/count")
    public ResponseEntity<Long> countDowngradings(DowngradingCriteria criteria) {
        log.debug("REST request to count Downgradings by criteria: {}", criteria);
        return ResponseEntity.ok().body(downgradingQueryService.countByCriteria(criteria));
    }
