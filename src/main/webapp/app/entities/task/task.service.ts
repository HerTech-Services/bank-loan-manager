...
export class TaskService {
  ...
  protected convertDateFromClient(task: ITask): ITask {
    const copy: ITask = Object.assign({}, task, {
      createdDate: task.createdDate && task.createdDate.isValid() ? task.createdDate.toJSON() : undefined,
      processDate: task.processDate && task.processDate.isValid() ? task.processDate.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((task: ITask) => {
        task.createdDate = task.createdDate ? moment(task.createdDate) : undefined;
        task.processDate = task.processDate ? moment(task.processDate) : undefined;
      });
    }
    return res;
  }
