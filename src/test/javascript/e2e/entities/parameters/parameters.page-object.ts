...
export class ParametersUpdatePage {
  ...
  paramValueJsonInput = element(by.id('field_paramValueJson'));
  ...
  async setParamValueJsonInput(paramValueJson: string): Promise<void> {
    await this.paramValueJsonInput.sendKeys(paramValueJson);
  }

  async getParamValueJsonInput(): Promise<string> {
    return await this.paramValueJsonInput.getAttribute('value');
  }
