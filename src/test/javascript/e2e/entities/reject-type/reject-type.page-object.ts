...
export class RejectTypeComponentsPage {
  createButton = element(by.id('jh-create-entity'));
  deleteButtons = element.all(by.css('jhi-reject-type div table .btn-danger'));
  title = element.all(by.css('jhi-reject-type div h2#page-heading span')).first();
...
export class RejectTypeUpdatePage {
  pageTitle = element(by.id('jhi-reject-type-heading'));
...
export class RejectTypeDeleteDialog {
  private dialogTitle = element(by.id('jhi-delete-rejectType-heading'));
  private confirmButton = element(by.id('jhi-confirm-delete-rejectType'));
