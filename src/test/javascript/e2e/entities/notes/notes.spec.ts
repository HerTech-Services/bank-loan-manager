...
describe('Notes e2e test', () => {
  ...
  it('should load Notes', async () => {
    ...
    expect(await notesComponentsPage.getTitle()).to.eq('bankLoanManagerApp.notes.home.title');
  ...
  it('should load create Notes page', async () => {
    ...
    expect(await notesUpdatePage.getPageTitle()).to.eq('bankLoanManagerApp.notes.home.createOrEditLabel');
  ...
  /* it('should create and save Notes', async () => {
        ...
        await promise.all([
            notesUpdatePage.setDescriptionInput('description'),
            notesUpdatePage.setCreatedDateInput('01/01/2001' + protractor.Key.TAB + '02:30AM'),
            notesUpdatePage.userSelectLastOption(),
            notesUpdatePage.engagementSelectLastOption(),
        ]);

        expect(await notesUpdatePage.getDescriptionInput()).to.eq('description', 'Expected Description value to be equals to description');
        expect(await notesUpdatePage.getCreatedDateInput()).to.contain('2001-01-01T02:30', 'Expected createdDate value to be equals to 2000-12-31');
  ...
  /* it('should delete last Notes', async () => {
        ...
        expect(await notesDeleteDialog.getDialogTitle())
            .to.eq('bankLoanManagerApp.notes.delete.question');
