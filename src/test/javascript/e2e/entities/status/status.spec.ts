...
describe('Status e2e test', () => {
  ...
  it('should create and save Statuses', async () => {
    ...
    const selectedIsEnabled = statusUpdatePage.getIsEnabledInput();
    if (await selectedIsEnabled.isSelected()) {
      await statusUpdatePage.getIsEnabledInput().click();
      expect(await statusUpdatePage.getIsEnabledInput().isSelected(), 'Expected isEnabled not to be selected').to.be.false;
    } else {
      await statusUpdatePage.getIsEnabledInput().click();
      expect(await statusUpdatePage.getIsEnabledInput().isSelected(), 'Expected isEnabled to be selected').to.be.true;
    }
